...
# JWT configuration — fail hard if secret is missing or default
SECRET_KEY = os.environ.get("APP_SECRET_KEY")
if not SECRET_KEY or SECRET_KEY == "change-me-in-production":
    raise RuntimeError(
        "APP_SECRET_KEY must be set to a unique random value. "
...
def verify_refresh_token(
    ...
    Returns None if the token is invalid, expired, revoked, or the
    user-agent doesn't match the original client.
    """
    if db is None:
        from app.database import SessionLocal
        db = SessionLocal()
    ...
    try:
        ...
        # H5: Bind refresh token to client — reject if user-agent changed
        if user_agent and rt.user_agent and rt.user_agent != user_agent:
            return None
...
def get_current_user_from_token(
    ...
    Only accepts 'Bearer <jwt>' format. Legacy 'bearer-{id}-{username}'
    format has been removed (C1 security fix).
    """
