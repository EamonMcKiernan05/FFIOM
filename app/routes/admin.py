...
All endpoints require admin authentication (C2 security fix).
"""
...
def seed_sample_data(
    ...
    # Create sample players for each team (no position data)
    player_names = ["Smith", "Jones", "Brown", "Wilson", "Taylor", "Bloggs", "White",
                    "Green", "Black", "Gray", "Clark", "Hall", "Moore", "Allen", "Young",
                    "King", "Wright", "Scott", "Baker", "Adams", "Hill", "Nelson",
...
def manual_sync_fixtures(admin: User = Depends(require_admin)):
...
def manual_process_gameweek_end(admin: User = Depends(require_admin)):
...
def manual_sync_and_score(admin: User = Depends(require_admin)):
