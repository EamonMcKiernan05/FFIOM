...
State-changing endpoints derive the user from the JWT token (C3 IDOR fix).
"""
import secrets
...
def create_mini_league(
    ...
    """Create a new mini-league. The creator becomes the admin."""
...
def get_user_leagues(
    ...
    for m in memberships:
        ...
        leagues.append({
            ...
            "name": ml.name,
            "code": ml.code,
            "is_admin": ml.admin_user_id == user.id,
            "is_h2h": ml.is_h2h,
...
def calculate_league_ranks(
    ml_id: int,
    user: User = Depends(get_current_user_from_token),
    db: Session = Depends(get_bound_db),
):
    """Calculate and update ranks for a mini-league (admin only)."""
    ml = db.query(MiniLeague).filter(MiniLeague.id == ml_id).first()
    if not ml:
        raise HTTPException(status_code=404, detail="League not found")

    if ml.admin_user_id != user.id:
        raise HTTPException(status_code=403, detail="Only the admin can recalculate ranks")
...
def _build_ml_response(ml: "MiniLeague", db: Session) -> dict:
    ...
    # Sort by rank asc, then by points desc for ties
